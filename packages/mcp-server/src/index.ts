...
要求→機能→仕様のトレーサビリティをグラフ構造で管理します。1つの要求から複数の機能、1つの機能から複数の仕様をぶら下げられます。
...
2. **既存ノードとの重複・矛盾・関連性を分析する**
   - ユーザーの要望に対して、既存のneed/feature/spec等と重複していないか確認
   - 矛盾がある場合は具体的に指摘する
   - 関連するノードがあれば言及する
...
- feature: 機能（needを具体化した機能単位）
- spec: 仕様（featureを満たす技術仕様）※ featureの子ノード
...
2. 返却されたノード（仕様等）の内容が実装と乖離していないか確認する
